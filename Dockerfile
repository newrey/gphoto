...
RUN sed -i -e "s|http://archive.ubuntu.com/|http://mirrors.ustc.edu.cn/|g" /etc/apt/sources.list
RUN apt-get update && \
    apt-get install -y \
    libsm6 \
    libboost-all-dev \
...
COPY modules/places365_model.tar.gz /code/api/places365
